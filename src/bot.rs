...
impl BotController {
    ...
    pub async fn run(&self) {
        ...
        let ws_url = "ws://127.0.0.1:8080/v1/receive/***REDACTED***";
        let (ws_stream, _) = match connect_async(ws_url).await {
            Ok(s) => s,
            Err(e) => {
                eprintln!("Failed to connect to Signal WebSocket. Is the docker container running? {}", e);
                return;
            }
        };
            ...
            if let Ok(WsMessage::Text(text)) = msg {
                if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&text) {
                    ...
                    if let Some(envelope) = parsed.get("envelope") {
                        ...
                        timestamp = envelope.get("timestamp").and_then(|t| t.as_u64()).unwrap_or(0);

                        // Normal messages
                        if let Some(data_message) = envelope.get("dataMessage") {
                            if let Some(msg_text) = data_message.get("message").and_then(|m| m.as_str()) {
                        ...
                        if let Some(sync_message) = envelope.get("syncMessage") {
                            if let Some(sent_message) = sync_message.get("sentMessage") {
                                if let Some(msg_text) = sent_message.get("message").and_then(|m| m.as_str()) {
                                    text_content = Some(msg_text.to_string());
                                }
                            }
                        }
    ...
    async fn handle_command(&self, cmd: &str, args: &str) -> String {
        match cmd {
            "/status" => {
                match self.garmin_client.fetch_data() {
                    Ok(data) => {
                        let bb = data.recovery_metrics.as_ref().and_then(|m| m.current_body_battery).map(|v| v.to_string()).unwrap_or_else(|| "N/A".to_string());
                        let sleep = data.recovery_metrics.as_ref().and_then(|m| m.sleep_score).map(|v| v.to_string()).unwrap_or_else(|| "N/A".to_string());
                        let today = chrono::Local::now().format("%Y-%m-%d").to_string();
                        let today_workouts: Vec<_> = data.scheduled_workouts.iter().filter(|w| w.date.starts_with(&today)).collect();
                        
                        let planned_str = if today_workouts.is_empty() {
                             "None - Rest Day!".to_string()
                        } else {
                             today_workouts.iter().map(|w| format!("{} ({})", w.title, w.sport)).collect::<Vec<_>>().join(", ")
                        };
            ...
            "/generate" => {
                 match crate::run_coach_pipeline(
                     ...
                     Ok(_) => "✅ Successfully generated and scheduled the week's workouts!".to_string(),
                     Err(e) => format!("Failed to generate workout: {}", e),
                 }
            }
            "/macros" => {
                 if args.is_empty() {
                    "Please provide macros. Example: /macros 2500 150 (calories protein)".to_string()
                } else {
                    ...
                    if parts.len() >= 2 {
                        ...
                        if let (Ok(kcal), Ok(protein)) = (kcal_str.parse::<i32>(), protein_str.parse::<i32>()) {
                            let today = chrono::Local::now().format("%Y-%m-%d").to_string();
    ...
    async fn send_reply(&self, recipient: &str, text: &str) {
         let send_req = SendMessageReq {
             ...
             number: "***REDACTED***".to_string(),
             recipients: vec![recipient.to_string()],
         };

         let client = reqwest::Client::new();
         let res = client.post("http://127.0.0.1:8080/v2/send")
             .json(&send_req)
             .send()
             .await;
