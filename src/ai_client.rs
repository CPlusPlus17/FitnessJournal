    ...
    pub async fn generate_workout(&self, prompt: &str) -> Result<String> {
        let request_body = GeminiRequest {
            system_instruction: Some(SystemInstruction {
                parts: vec![Part { text: "You are an elite Multi-Sport Coach. Follow instructions precisely.".to_string() }]
            }),
            contents: vec![
                Content {
                    role: "user".to_string(),
        ...
        let gemini_response: GeminiResponse = response.json().await.context("Failed to parse Gemini JSON")?;
