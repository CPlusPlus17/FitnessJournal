...
use std::path::Path;
use crate::models::{GarminActivity, GarminSetContainer};
...
impl Database {
    ...
    pub fn get_progression_history(&self) -> Result<Vec<String>> {
        ...
        while let Some(row) = rows.next()? {
            ...
            history.push(format!("- **{}**: {}kg x {} ({})", name, weight, reps, date));
        }

        Ok(history)
    }

    pub fn get_progression_history_raw(&self) -> Result<Vec<(String, f64, i32, String)>> {
        let mut stmt = self.conn.prepare(
            "SELECT exercise_name, MAX(weight), reps, date 
             FROM exercise_history 
             GROUP BY exercise_name 
        ...
        while let Some(row) = rows.next()? {
            ...
            history.push((name, weight, reps, date));
        }

        Ok(history)
    }

    pub fn get_recent_muscle_heatmap(&self, days: u32) -> Result<Vec<crate::models::ExerciseMuscleMap>> {
        ...
        while let Some(row) = rows.next()? {
            ...
            let muscles = match name.as_str() {
                "BENCH_PRESS" | "PUSH_UP" => vec!["chest".to_string(), "triceps".to_string(), "front-deltoids".to_string()],
                "ROW" => vec!["upper-back".to_string(), "lower-back".to_string(), "biceps".to_string(), "back-deltoids".to_string()],
                "PULL_UP" | "PULL_DOWN" => vec!["upper-back".to_string(), "biceps".to_string(), "back-deltoids".to_string()],
                "SQUAT" | "LUNGE" => vec!["quadriceps".to_string(), "gluteal".to_string(), "hamstring".to_string(), "calves".to_string()],
                "DEADLIFT" => vec!["hamstring".to_string(), "gluteal".to_string(), "lower-back".to_string(), "forearm".to_string(), "trapezius".to_string()],
                "CALF_RAISE" => vec!["calves".to_string()],
                "SHOULDER_PRESS" | "FRONT_RAISE" | "LATERAL_RAISE" => vec!["front-deltoids".to_string(), "back-deltoids".to_string(), "triceps".to_string()],
