...
export default function MuscleMap() {
    ...
    const activeMuscleCount = Object.keys(muscleFrequencies).length;
    ...
    }, [muscleToExercises, expanded]);

    if (loading) {
        return (
            <div className="glass-panel p-6 flex items-center justify-center min-h-[80px]">
                <div className="text-gray-400 animate-pulse text-sm">Loading Muscle Heatmap...</div>
            </div>
        );
    }

    return (
        <div className="glass-panel p-5 relative overflow-hidden bento-full section-reveal">
            {/* Header - always visible */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10">
                <div className="flex items-center gap-4">
                    <div>
                        <h3 className="text-base font-bold tracking-tight text-white">Muscle Fatigue Heatmap</h3>
                        <p className="text-gray-500 text-xs mt-0.5">
                            {activeMuscleCount > 0 ? `${activeMuscleCount} muscle groups active` : 'No active data'} · Last 14 days
                        </p>
                    </div>
                </div>
                <div className="flex items-center gap-3 mt-3 md:mt-0">
                    {expanded && (
                        <div className="flex bg-white/[0.03] p-1 rounded-xl border border-white/8 backdrop-blur-sm">
                            <button
                                onClick={() => setGender('male')}
                                className={`px-3 py-1 text-xs font-medium rounded-lg transition-all ${gender === 'male' ? 'bg-red-500/15 text-red-400 border border-red-500/25' : 'text-gray-400 hover:text-gray-300 border border-transparent'}`}
                            >
                                Male
                            </button>
                            <button
                                onClick={() => setGender('female')}
                                className={`px-3 py-1 text-xs font-medium rounded-lg transition-all ${gender === 'female' ? 'bg-red-500/15 text-red-400 border border-red-500/25' : 'text-gray-400 hover:text-gray-300 border border-transparent'}`}
                            >
                                Female
                            </button>
                        </div>
                    ...
                    <button
                        ...
                        className="text-xs bg-white/5 hover:bg-white/10 px-4 py-2 rounded-xl border border-white/8 transition-all hover:border-white/15 text-gray-300 font-medium flex items-center gap-1.5"
                    >
                        {expanded ? (
                            <>
            ...
            <div className={`collapsible-content ${expanded ? 'is-expanded' : ''}`}>
                <div>
                    <div className="mt-6">
                        {/* Ambient glow behind */}
